...
| \longrightarrow (\text{implication})                | function type          |
| \forall (\text{universal quantification})   | \Pi (dependent type)     |
| \exists (\text{existential quantification}) | \Sigma (dependent sum type) |

Take for example the formula \(A \wedge P \longrightarrow P\). Informally, when we have evidence for both \(A\) and \(P\) then we can get a \(P\), and similarly for types if we can give a pair \((A,P)\) then we can get the inhabitant \(P\).
...
For proposition such as \(P \wedge (Q \vee R) \longrightarrow ((P \wedge Q) \vee (P \wedge R)\):
