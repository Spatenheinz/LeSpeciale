...
Depending on the specific domain, the safety policy could include constraints such as no out-of-bounds memory operations and termination of loops and the likes that might require programs to not corrupt the consumer.
The code producer then uses the safety policy to certify the program in a compilation stage of the untrusted code.
