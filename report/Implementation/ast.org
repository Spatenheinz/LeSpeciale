Despite being similar to C and C++ in syntax, Rust provides a much richer typesystem that allow us to create enumerations with fields, a.k.a Sum types.
...
own the string.
Having terms parameterized by the ~Id~ type allow for easy convertion to De Bruijn levels instead of
string identifiers.
