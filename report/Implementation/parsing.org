...
#+end_src
We parse the different aspects of a binder, indentifier, binding term and the bound term, and then construct the appropriate binder.

For terms in general we must ensure the parser to be robust and able to handle arbitrary nesting of parentheses.
So, when parsing nonterminal terms, we first parse an open parenthesis followed by ~parse_term_~. We then try to parse a binder followed by a closed parenthesis\footnote{here binders also include : and \^}. If we fail it parse a term or, if we can parse multiple terms, an application, as shown below.
#+begin_src rust
