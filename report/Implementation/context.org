Signatures \Sigma are denoted as ~GlobalContext~ while \Gamma is used for the ~LocalContext~.
...
- ~Val~ is used for extending the environment in evaluation. This constructor can never occur in \Sigma but may occur in $\Gamma$.
...
On the other hand if one tries to get the type of ~Val~ an error occurs.

The local context exposes a much similar interface but with a different underlying data structure.
