We define two functions for typechecking. One takes a term ~term~ and a ~Value~ ~t2~ and check against it.
...
This process is two-fold. Firstly we do a value comparison between ~t1~ and ~t2~. If not succesful we convert them
to their canonical form using reification and check for equality.
Generally we cannot compare values, as functions such as ~Pi~ has a closures inside it that cannot easily be compared.
...
We must do it this way, as holes cannot be filled after reading back values as a hole might occur in multiple places and we must ensure
