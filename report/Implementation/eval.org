...
More interesting, if the domain of a \Pi is a sidecondition, we evaluate the side condition and the target and check for equivalence.
Lastly the result is inserted in the local context and the body is evaluated.
This is what enables execution of side conditions in the typechecking.

Side conditions on the other hand may only be evaluated, when part of \Pi type.
We evaluate the sidecondition language by the ~sc~ function, which follow the operational semantics in Figure \ref{fig:operational}.
