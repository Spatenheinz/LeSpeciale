...
If that is the case, we infer the type of the side condition and check it to have the same type as type of the second part of the pair. We create a Run type ~val~ which will be bound in the inference of the ~b~.
...
First we infer the function point, which is saved in a mutable variable updated each iteration of the following loop.
