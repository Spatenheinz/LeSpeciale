\footnote{hvad skal jeg skrive som introducerende tekst}The operational semantics is show in Figure \ref{fig:operational}.
The operational semantics are under the judgement of
\(\Delta \vdash \judge{1}{S}{T}{n} \), where \Delta describes all program definitions and \(\sigma_1\) and \(\sigma_n\), describes states mapping symbols to markings, where \(\sigma_1\) is the state $S$ is evaluated under, and \(\sigma_n\) is the state where $S$ has been evaluated to $T$.
...
- the two rules /IFEQUAL-T/ and /IFEQUAL/ describes a standard semantic for equality checks. Two terms \(S_1'\) and \(S_2'\) are considered equivalent with respect to $\beta\eta$-equivalence.
