...
We use ~x~ to be a metavariable ranging over the set of variables that might occur in terms,
~c~ to denote constants in terms, i.e. free variables.
...
Primitives and keywords of the side condition language are represented in *bold*.
~z~ and ~q~ are metasymbols representing integers and rationals.
$*$ represents a hole, which might be any term.
...
\begin{figure}[h!]
\begin{align*}
K \; ::=& \; \mathbf{type} \; | \; \mathbf{type}^c \; | \; \mathbf{kind} \; | \; \pilf{x}{A}{K} \; | \; \mathbf{mpz} \; | \mathbf{mpq} \\
