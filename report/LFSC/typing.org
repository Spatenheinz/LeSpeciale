...
\begin{equation*}
\inference[APP-SC ]{ \context \synth{M}{\Pi x : \{S \; O\}. \ B} & \context \tycheck{N}{A} }
...
Typechecking of sideconditions occurs in /PI-SIDE/ of Figure \ref{tylfsc}.
We follow the structure of the language reference \cite{}.
Numbers, and variables follows similar rules as for the term language.
Similarly Let expressions \(\mathbf{let} x S T\) work as its term counterpart.
Applications are a bit more tricky.
Applications must be fully applied but at its core construct it should follows.
\begin{equation*}
\inference[SCAPP ]{\context \synth{S}{\Pi x : A. B} & \context \synth{T}{C} & x \notin FV(B) }
{\context \synth{S \; T}{B}}
\end{equation*}
Again the operator should be a pi type. But we further require that /x/ does not occur free in /B/.
This is essentially to ensure that parameters to a side condition program must not occur in the other parameters or in ty return type.
Below the 3 cateogrical constructs of side conditions are represented.

The inference of /DO/ is similar to the rule for let except we dont bind anything.
/MARKVAR/ will simply return the type the inner sidecondition.
/IFMARKED/ will check the "marked" sidecondition /S/ and that it is a symbolic value aswell as checking that the two branches have the same type.
 #+begin_export latex
\begin{figure}[h!]
\begin{equation*}
\inference[IFMARKED ]{\context \synth{S}{TODO A} & \synth{T_1}{B} & \synth{T_2}{B}}
...
The reason *fail* must take an argument is that we have no apparent way to polymorphically check its type from the rules.
...
given the scrutinee \(S_1\) can be inferred as \(A\) then for all match cases \(P_i\) must also be inferreable as /A/, whilst all branches \(T_i\) must be inferreable to the same type /B/. \(\mathbf{ctx}(P_i)\) describes the context created from \(P_i\). concretely
