...
Notice here that both applications of ~and_elim~ are provided with 4 arguments, even though the type suggest, that it should take 5. However from the typing rules the inner side condition of Figure \ref{fig:tylfsc} will be evaluated,
hnce when all types are checked ~nary_extract~ is run.
