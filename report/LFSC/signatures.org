...
\end{align*}
#+end_export
The notion of inference here describes the type-level semantic of an object, meaning if there is a judgement from $M$ that gives $A$, then $M \Rightarrow A$.
...
Figure \ref{fig:validctx} depicts validity of cntexts. An empty context is valid under \Sigma given the validity of \Sigma. For concatenation $x$ must not occur in \Sigma but can
