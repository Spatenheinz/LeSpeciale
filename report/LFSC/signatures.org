...
we write them as such and use \Sigma, \Sigma' to denote the concatenation of the two signatures \Sigma and \Sigma' and similarly for contexts. \(L : R\) denotes that L has type R.
#+begin_export latex
\begin{align*}
...
The validity of signatures is as follows, the empty signature is valid. Then Signature may contain either a
value or a type level constant of a type or kind respectively.
#+begin_export latex
\begin{figure}[h!]
\begin{equation*}
\inference[EMPTY-SIG ]{}
...
An empty context is valid under \Sigma given the validity of \Sigma. For a \(x : A\) to be part of a valid context, \(A\)
must be of kind * and x must not occur in the domain of the signature.
#+begin_export latex
\begin{figure}[h!]
\begin{equation*}
