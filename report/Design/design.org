...
Section \ref{} will then shift the focus to the main product of this project, namely the proof checker and describe
...
it should be clear that eBPF and the verifier has some clear similarities with the PCC.
...
Having a proof checker that implements a sound approach to proof checker, will both give a higher certainty in safety aswell as more acceptable programs, because it does not have to be as conservative.

*** How to add PCC to eBPF
To realize PCC in the linux kernel we must extend it in some way.
There are mainly four ways, we can extend the Linux kernel by the documentation\cite{}.
...
Option 3, likewise imposes a responsibility mismatch. All interactions with the eBPF subsystem goes through the syscall and having a seperate system call to validate the code
and load is not optimal. Likewise the proof checker cannot substitute the entire loading process but only the verifier, and here maybe not all of the checks can be completely removed.
For instance we might still require capabilities to be checked seperately, but include memory alignment in verification condition. On the other hand capabilities is just a boolean flag, which can easily be
...
*** Certifications
As previously mentioned we only implement part of the PCC in this report.
And this implementation should also just be seen as a prototype.

For the general architecture we consider a certification format based on formal logic of verification conditions based on predicate transformer semantics\cite{}.
...
We have in this work considered two output formats/languages, Alethe\cite{} and Logical Framework with Side Conditions (LFSC)\cite{lfsc},
...
for instance rule 20 is reflexivity often denoted as ~refl~ which states equality after applying a context.
...
This metaframework allows for the security policy to be established by signatures, which encodes similar rules to the once defined in Alethe.
...
Another important feature of an certification checker in the kernel is that it should be both memory and time efficient.
...
LFSC has a proof checker written in C++ and its code is merely 5000 lines\cite{}, and new signatures requires no modification to the code.
...
Overall LFSC provides a better basis for the usescase.
...
The specific formula that needs to be proved are directly imbedded in an LFSC proof.
...
This also mean that we can reuse some of the code used to implement the proof checker for the VC generation.
